...
const MemoryUtilizationPieChart: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
         `http://${VITE_REMOTE_IP}:8100/metrics/api/v1/query?query=100*(1-((node_memory_MemFree_bytes%2Bnode_memory_Cached_bytes)%2Fnode_memory_MemTotal_bytes))`
      );
  ...
  useEffect(() => {
    ...
    const colorScale = d3
      .scaleLinear<string>()
      .domain([0, 100])
      .range(["#89D8A8", "#1081E4"]); // Gradient from light green to deep blue
...
export default MemoryUtilizationPieChart;
