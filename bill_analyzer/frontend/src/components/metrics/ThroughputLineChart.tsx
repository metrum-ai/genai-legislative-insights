...
// File: src/components/ThroughputLineChart.tsx
...
const fetchThroughputData = async (): Promise<number | null> => {
  try {
    const response = await fetch(
       `http://${VITE_REMOTE_IP}:8100/metrics/api/v1/query?query=sum(vllm:avg_generation_throughput_toks_per_s{instance=~'vllm_serving_0:8000|vllm_serving_1:8000'})`
    );
...
const renderLineChart = (
  ...
  svg
    ...
    // .call(d3.axisBottom(x).tickFormat(null));
    .call(
      d3
        .axisBottom(x)
        .ticks(6)
...
const ThroughputLineChart: React.FC = () => {
  const chartRef = useRef<HTMLDivElement>(null);
  const [data, setData] = useState<DataPoint[]>([]);

  useEffect(() => {
    const updateData = async () => {
      const newValue = await fetchThroughputData();
      if (newValue !== null) {
        setData((prevData) => {
  ...
  return <div ref={chartRef} id="throughput-line-chart"></div>;
};
